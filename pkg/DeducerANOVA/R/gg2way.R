...
gg2way2 <- function(data,exclude,x,y,xwithin = FALSE, split = NULL,x_lab = NULL, y_lab = NULL, split_lab = NULL)
	{
		if(is.null(split)) split = "NULL"
		if(length(split) == 0) split = "NULL"
		if(!is.null(exclude)) data=data[-exclude,]
		xvar <- data[,match(x,names(data)]
		yvar <- data[,match(y,names(data)]
		splitvar <- data[,match(split,names(data)]
		dev.new()
		g <- ggplot() + theme_bw() +
		geom_errorbar(aes(y = yvar, x = xvar, linetype = splitvar, group = splitvar),
			data=data, width = 0.4, fun.data = 
			x = data[,match(x,names(data)], 
			mean_cl_normal, conf.int = 0.95, stat = 'summary'
			"data=",data,",width = 0.4, fun.data = ,",
			"position = position_dodge(width = 0.5))+\n",
		 "geom_point(aes(x = ", x, ", y = ",y, ", shape = ",split, ", group = ",split,"),",
			"data=",data,",size = 3.0, fun.data = mean_cl_normal, conf.int = 0.95,stat = 'summary', position = position_dodge(width = 0.5))",sep="")
